...
import { employeeSchema } from "@/components/schemas/employeeSchema";
import axios from "axios";
import { toast } from "sonner";
import { PropTypes } from "prop-types";

function EmployeeForm({ open, setOpen, setShowPaymentScreen }) {
  ...
  const handleFileChange = (event, setFieldValue) => {
    ...
    if (selectedFile) {
      const isValid = employeeSchema.fields.file.isValidSync(selectedFile);
      if (isValid) {
  ...
  const handleDrop = (event, setFieldValue) => {
    ...
    if (droppedFile) {
      const isValid = employeeSchema.fields.file.isValidSync(droppedFile);
      if (isValid) {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    try {
      ...
      const userApplied = await axios.get(
        `${process.env.NEXT_PUBLIC_URL}/employee-forms?filters[email][$eq]=${values.email}`
      );
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_URL}/employee-forms`,
        postData
      );
      ...
      setTimeout(() => {
        setLoader(false);
        // toast.success("Successfully Registered for the Courese", {
        //   cancel: {
        //     label: "Ok",
        //   },
        //   position: "top-center",
